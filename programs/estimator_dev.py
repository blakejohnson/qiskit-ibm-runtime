...
# from programs.pec_estimator import PECEstimator
from pec_runtime.primitives import Estimator as PECEstimator
...
class Constant:
    ...
    PEC_DEFAULT_NUM_SAMPLES: int = 25
...
def main(
    ...
    enable_pec = resilience_settings.pop("enable_pec", False)
    ...
    use_pec = resilience_settings.get("level", 0) == Constant.PEC_RESILIENCE_LEVEL and enable_pec

    if use_pec:
        transpilation_settings["basis_gates"] = backend.configuration().basis_gates
        if isinstance(circuits, dict):
            keys = list(circuits.keys())
            circuit_indices = [keys.index(id) for id in circuit_ids]
            circuits = circuits.values()
        estimator = PECEstimator(
            backend=backend,
            circuits=circuits,
            observables=observables,
        ...
        num_samples = resilience_settings.get("num_samples", Constant.PEC_DEFAULT_NUM_SAMPLES)
        run_options["num_samples"] = num_samples
    else:
        estimator = Estimator(
            backend=backend,
            circuits=circuits,
