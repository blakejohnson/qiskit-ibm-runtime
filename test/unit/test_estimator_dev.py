...
from programs.estimator_dev import CircuitCache, Estimator, PauliTwirledMitigation, main, Constant

from .mock.mock_cache import MockCache


def get_backend(resilience_level: int, with_noise: bool = False) -> Backend:
    """Returns an appropiate backend for a given resilience level and noise
    Args:
        resilience_level: The resilience level. When `resilience_level` is 2 (PEC),
        the backend needs to have a coupling map define, therefore an Aer backend
        cannot be used
        with_noise: whether or not the backend should be noisy
...
class TestEstimatorMainCircuitIndices(unittest.TestCase):
    ...
    def test_no_noise_multiple_params(self, resilience_level):
        ...
        if resilience_level == 0:
            ...
            for i in range(0, len(result["metadata"])):
    ...
    # TO-DO: add resilience_level = Constant.PEC_RESILIENCE_LEVEL when bug is fixed in PEC Estimator
    ...
    def test_identity(self, noise, resilience_level, shots):
        ...
        backend = get_backend(resilience_level, noise)
        # backend = FakeBogota()
        ...
        if resilience_level == 0:
            ...
            for i in range(0, len(result["metadata"])):
                metadata = result["metadata"][i]
                self.assertEqual(metadata["layout_qubits"], [4, 3, 2, 1, 0])
                self.assertEqual(metadata["noise_scale"], 0)
                self.assertEqual(metadata["shots"], shots)
                self.assertEqual(metadata["samples"], Constant.PEC_DEFAULT_NUM_SAMPLES)
                if noise:
                    self.assertGreater(metadata["gamma"], 1.0)
                else:
                    self.assertEqual(metadata["gamma"], 1.0)
